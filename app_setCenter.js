...
var ind = 0, h = 0, k = 0, theta = -70, alpha = 0, beta = 0; dLng = 0, dLat = 0, time = 0, steerTime = 0, latFact = 0, lngFact = 0, Diag = 0;
...
acc = {
    ...
    rev: function() {
        if (this.lin != -15 && this.lin != -0.2) time = 0;
        resSet = true;
        if (Math.trunc(vel.lin) > 0 ) this.lin = -15;
        else this.lin = -0.2;
    },
    EB: function() {
        if (this.lin != -115 && this.lin != 65) time = 0;
        resSet = true;
...
function traverseBy(speed) {
    ...
    map.setCenter ({ 
        lat: (map.center.lat() + h), 
        lng: (map.center.lng() + k) 
    });
    //map.panBy(k, h);
};
...
window.onload = function () {
    ...
    setInterval( function dropTime() {
        ...
        traverseBy(vel.lin * 0.001); moveDest();
